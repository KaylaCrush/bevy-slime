...
use crate::pheromones::{make_pheromone_array_images, PheromoneArrayImages};
use crate::resources::{GlobalUniforms, PheromoneUniforms, SlimeSimImages, SlimeSimRunConfig, SIZE, DISPLAY_FACTOR};
...
pub fn setup(
    ...
    commands.spawn((
        Sprite { image: image0.clone(), custom_size: Some(SIZE.as_vec2()), ..default() },
        Transform::from_scale(Vec3::splat(DISPLAY_FACTOR as f32)),
    ));
    commands.spawn(Camera2d);

    commands.insert_resource(SlimeSimImages { texture_a: image0, texture_b: image1, temp_texture: image2 });
    ...
    let layer_params: Vec<PheromoneLayerParam> = vec![
        PheromoneLayerParam { diffusion: 0.5, decay: 0.8, _pad0: 0.0, _pad1: 0.0, color: Vec4::new(1.0, 0.0, 0.0, 1.0) },
        PheromoneLayerParam { diffusion: 0.3, decay: 0.6, _pad0: 0.0, _pad1: 0.0, color: Vec4::new(0.0, 1.0, 0.0, 1.0) },
        PheromoneLayerParam { diffusion: 0.7, decay: 0.9, _pad0: 0.0, _pad1: 0.0, color: Vec4::new(0.0, 0.0, 1.0, 1.0) },
    ];
    ...
    // Run config
    commands.insert_resource(SlimeSimRunConfig { run_copy_and_input: true, run_diffuse: true, run_agents: true });
    ...
    agents::init_agents(&mut commands, &render_device, SIZE, agents::NUM_AGENTS, species_count);

    // Species GPU buffer is uploaded by species::upload_species_to_gpu during Startup
}

pub fn switch_textures(images: Res<SlimeSimImages>, mut sprite: Single<&mut Sprite>) {
    if sprite.image == images.texture_a { sprite.image = images.texture_b.clone(); } else { sprite.image = images.texture_a.clone(); }
}
